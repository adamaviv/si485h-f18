...
1.  (15 points) Consider the `gdb` output for function `foo()` :
    ...
    -   At what address, in hexadecimal, could there be a potential
...
2.  (15 points) Consider the `gdb` output for function `foo()` :
    ...
    -   Consider executing this program’s `main()` function, which calls
...
6.  (15 points) Consider the following assembly program written in `asm`
    ...
    -   How does the output of the program change if at `MARK 1` the 0x1
        were changed to 0x2?
...
7.  (15 points) Consider the copiled and assembled shell code:
    ...
    -   Complete the stack diagram prior to the first interupt assuming
    ...
    -   If we were to package of the bytes into a string and call it
        like so:
...
8.  (15 points) Consider the following shell code:
    ...
    -   At the the `pop esi` instructino at address `0x8048062`, what
        address will be stored in the `esi` register?

    -   Explain how this shell code avoids fixed references?

    -   This shell code still hass NULL bytes! Rewrite the `x86` such
...
9.  (20 points) Consider the following C source code and dissambly of
    ...
    -   If you were to explot the program using the following method,
    ...
    -   With the right padding, will the above method work? Explain why
        or why not.

    -   Consider smashing the stack using the following method, how many
    ...
    -   In some cases, the second padding is needed. Give one example
        case.
